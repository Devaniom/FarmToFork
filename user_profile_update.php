...
if (isset($_POST['update_profile'])) {
   ...
   $check_pass_query->execute([$user_id]);
   $fetch_pass = $check_pass_query->fetch(PDO::FETCH_ASSOC);

   if (password_verify($_POST['update_pass'], $fetch_pass['password'])) {
      if ($_POST['new_pass'] == $_POST['confirm_pass']) {
         $new_hashed_pass = password_hash($_POST['new_pass'], PASSWORD_DEFAULT);
         $update_pass_query = $conn->prepare("UPDATE `users` SET password = ? WHERE id = ?");
         $update_pass_query->execute([$new_hashed_pass, $user_id]);
...
<head>
   ...
   <title>FramToFork | Update Profile</title>
